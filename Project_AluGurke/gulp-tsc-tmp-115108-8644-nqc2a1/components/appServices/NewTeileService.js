...
var NewTeileService = (function () {
    function NewTeileService($rootScope, bestellService) {
        var _this = this;
                ...
                if (this.alleKaufteile[i].id == artikel[j]._id) {
                    ...
                    if (this.bestellService.neuBestellungen['k' + this.alleKaufteile[i].id].length != 0) {
                        this.alleKaufteile[i].teileWertNeu =
                            this.getKaufTeilTeileWertNeu(this.alleKaufteile[i].lagerMenge, this.alleKaufteile[i].teileWert, this.alleKaufteile[i].id);
                    }
                }
            }
        }
    };
    NewTeileService.prototype.getKaufTeilTeileWertNeu = function (lagerMenge, teileWertAlt, id) {
        var bestellKosten = 0;
        var bestellMenge = 0;
    ...
    NewTeileService.prototype.erzeugeErzeignisse = function () {
...
angular.module('app').factory('NewTeileService', ['$rootScope', 'BestellService', function ($rootScope, bestellService) { return new NewTeileService($rootScope, bestellService); }]);
